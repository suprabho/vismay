  ...
  return (
    ...
    >
      {!print && (
        ...
        >
          Slides preview · add ?print=1 to hide chrome
        </div>
      )}
      {slides.map(({ unit, center, zoom, pitch, bearing, pins, regions, heatmap }, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 flex" style={{ paddingTop: '64px', paddingBottom: '48px' }}>
              ...
              <div className="relative h-full flex flex-col" style={{ width: '50%', padding: '64px 80px' }}>
                ...
                {heading && (
                  <h2
                    ...
                    style={{ fontSize: '52px', lineHeight: 1.1, color: 'var(--color-text)' }}
                  >
                    {heading}
                  </h2>
                )}
                {subheading && (
                  <p
                    className="mb-6"
                    style={{ fontSize: '22px', lineHeight: 1.4, color: 'var(--color-muted)' }}
                  >
                    {subheading}
                  </p>
                )}
                <div className="flex-1 min-h-0 flex flex-col gap-4 overflow-hidden">
                  {unit.paragraphs.map((p, j) => (
                    <p
                      key={j}
                      style={{ fontSize: '20px', lineHeight: 1.55, color: 'var(--color-text)' }}
                    >
                      {stripMarkdown(p)}
                    </p>
                  ))}
