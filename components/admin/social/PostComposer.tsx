...
export function PostComposer({
  ...
  // Reset asset selection when channel changes & current kind is no longer allowed
  useEffect(() => {
  ...
  // Load share cards when story changes (only if needed)
  useEffect(() => {
    if (!draft.storySlug) {
      setShareCards([])
      return
    ...
    fetch(`/api/admin/social/share-cards/${draft.storySlug}`)
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
  ...
  async function prefillText() {
    ...
    setPrefilling(true)
    try {
      const r = await fetch('/api/admin/social/preview-text', {
        method: 'POST',
  ...
  async function save(asStatus: 'draft' | 'scheduled') {
    ...
    try {
      ...
      const url = editing
        ? `/api/admin/social/posts/${editing.id}`
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-stretch justify-end" onClick={onClose}>
      ...
      >
        <div className="shrink-0 px-4 py-3 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-sm font-semibold">{editing ? 'Edit post' : 'New post'}</h2>
