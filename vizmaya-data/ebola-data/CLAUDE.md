...
This folder follows the `vizmaya-data/<story>/` layout used elsewhere in
the monorepo (see `vizmaya-data/coke-studio/` for the sibling pattern).

- `*.csv` — flat at the root of this folder. Each CSV has a header row and
  an entity per row.
- `charts/*.json` — ECharts option objects, one per chart. Each carries a
  leading `_meta` block (`id`, `title`, `subtitle`, `source`, `note`)
  describing what it is. The `_meta` key is ignored by ECharts at render.
- `story.yaml` — the story manifest. Section order, dataset registry,
  chart registry, source tier list, caveats. Single source of truth.
- `REPORT.md` — long-form editorial write-up. Every numerical claim cites
  a source URL.
- `README.md` — one-screen overview matching the `coke-studio/README.md`
  pattern (file table + conventions).
- `INGEST_NOTES.md` — per-figure provenance: where each number came from,
  source-page incompleteness flags, gotchas. Matches the `coke-studio/INGEST_NOTES.md` convention.
- The standalone HTML preview lives at the repo root (`/index.html`) and
  embeds its own copies of the data; it is the click-to-preview artifact,
