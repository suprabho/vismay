...
| 2026-07-05 | comtrade (bilateral) | full backfill (`--full`, local dev machine) | 1,660,656 | `trade_bilateral_flows`: 20×19 pairs × HS2 × 2001–2025 × both flows, 52 API calls with zero errors (~29–35k rows per year-flow slice). DB count matches the importer total exactly; export/import split 826,316 + 834,340. Trade-web viz verified live in both lenses. |
| 2026-07-05 | comtrade (bilateral) | idempotency re-run | identical | `--year=2024 --flow=export` re-fetched the identical 29,078 rows (also matching the pre-migration dry-run) and upserted with the table total unchanged at 1,660,656. |
