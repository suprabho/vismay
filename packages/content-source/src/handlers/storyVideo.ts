...
export function createStoryVideoHandler(opts: StoryVideoHandlerOptions) {
  return {
    async GET(
      ...
      try {
        return await runStoryVideoGet(opts, req, params)
      } catch (err) {
        // Top-level safety net so any unexpected throw still surfaces as a
        // readable JSON {error}, never an opaque Next.js HTML 500 — the UI
        // displays `body.error ?? 'HTTP {status}'`, so without this the user
        // sees a blank "HTTP 500" with no clue what failed.
        const message =
          err instanceof Error ? err.message : 'story-video handler crashed'
...
async function runStoryVideoGet(
  opts: StoryVideoHandlerOptions,
  req: Request,
