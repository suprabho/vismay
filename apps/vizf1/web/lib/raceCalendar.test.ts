...
import { calendarDays, calendarWeeks, localRaceDate, raceDateLabel, raceDayLabel, racesByStatus, raceTimeLabel, raceWeekend, shiftDate, shiftMonth, utcRaceDate, weekCells, weekendOverlapsMonth, weekendRangeLabel } from './raceCalendar'
...
test('race day is two days after the stored weekend start, in the viewer time zone', () => {
  // Schedule rows carry the meeting start (Friday) and the race session's UTC time of day.
  const lasVegas = { date: '2026-11-20', time: '04:00:00Z' }
  assert.equal(utcRaceDate(lasVegas), '2026-11-22')
  assert.equal(localRaceDate(lasVegas, 'UTC'), '2026-11-22')
  assert.equal(localRaceDate(lasVegas, 'America/Los_Angeles'), '2026-11-21', 'a 04:00 UTC start is Saturday evening in Nevada')
  ...
  assert.equal(raceTimeLabel(lasVegas, 'America/Los_Angeles'), '20:00 GMT-8')
  assert.equal(raceTimeLabel({ date: '2026-10-02', time: '14:00:00Z' }, 'Asia/Kolkata'), '19:30 GMT+5:30')
  assert.equal(localRaceDate({ date: '2026-10-02', time: null }, 'America/Los_Angeles'), '2026-10-04', 'placeholders without a start time still put the race on the third day')
  assert.equal(raceDayLabel({ date: '2026-10-02', time: null }), 'Sun 4 Oct')
  assert.equal(raceTimeLabel({ date: '2026-10-02', time: null }), null)
})

test('a race weekend spans the stored start date and the next two days, in local time', () => {
  const weekend = raceWeekend({ date: '2026-10-02', time: '13:00:00Z' }, 'Asia/Kolkata')
  ...
  // A late UTC start that lands after midnight for the viewer shifts the whole weekend by a day.
  assert.deepEqual(raceWeekend({ date: '2026-10-23', time: '19:00:00Z' }, 'Asia/Kolkata').days, ['2026-10-24', '2026-10-25', '2026-10-26'])
  const vegas = raceWeekend({ date: '2026-11-20', time: '04:00:00Z' }, 'America/Los_Angeles')
  assert.deepEqual(vegas.days, ['2026-11-19', '2026-11-20', '2026-11-21'])
  const monthCrossing = raceWeekend({ date: '2026-10-30', time: '13:00:00Z' }, 'UTC')
  assert.equal(monthCrossing.end, '2026-11-01')
...
test('week cells merge weekend days into one spanning cell and clip at week and month edges', () => {
  const bahrain = { race: 'bahrain', weekend: raceWeekend({ date: '2026-10-02', time: '13:00:00Z' }, 'UTC') }
  ...
  const monday = { race: 'monday', weekend: raceWeekend({ date: '2026-10-10', time: '13:00:00Z' }, 'UTC') }
  ...
  const november = { race: 'nov', weekend: raceWeekend({ date: '2026-10-30', time: '13:00:00Z' }, 'UTC') }
