...
/**
 * Tiny markdown renderer scoped to the shape footshorts' worker/src/recap.ts
 * emits: h1/h2/h3, paragraphs, italic meta lines, and (optionally nested) bullet
 * lists with **bold**, *italic* / _italic_, and [links](url). Not a general
 * parser — it only handles what the recap generator produces, so the admin
 * doesn't need a markdown dependency for one view. Pure render, no client hooks.
 */

// Inline tokens. Links are listed first so a link starting with `[` wins over any
// `_`/`*` inside its URL (leftmost match).
