...
export default function DemoEditorClient({ demoId, initial, defaultContentYaml }: Props) {
  ...
  function save() {
    start(async () => {
      ...
      const res = await fetch(`/api/vizmaya/demos/${demoId}`, {
        method: 'PUT',
    ...
    <div className="flex-1 flex flex-col">
      <div className="border-b border-white/10 px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 min-w-0">
          <Link href="/vizmaya/demos" className="text-sm text-neutral-400 hover:text-white shrink-0">
            ← Demos
          </Link>
...
function ShareTab({
  ...
  useEffect(() => {
    fetch(`/api/vizmaya/demos/${demoId}/cards`)
      .then((r) => r.json())
  ...
  async function triggerRender() {
    setRendering(true)
    try {
      const res = await fetch(`/api/vizmaya/demos/${demoId}/render-share`, { method: 'POST' })
      const body = await res.json().catch(() => ({}))
      if (!res.ok) {
