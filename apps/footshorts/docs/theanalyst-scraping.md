...
| `theanalyst/matchDiscovery.ts` `readMatchesForVisibleDay` | fixture tiles: **markup changed 2026-09-10** (worked 2026-08-24 → 2026-09-09). The match-centre `<a href="…matchId=…">` is now a sibling inside the `<li class="…match-tile-shell …fixture-tile">` with only a screen-reader label ("Real Madrid v Internazionale (opens in a new tab)"); team names moved to `[class*="fixture-tile-team__name"]` nodes elsewhere in the `<li>` (display short names — "Inter" — plus a separate `__sr` full name). The old reader required team nodes *inside* the anchor and silently returned zero matches for every day (the cron logged "0 listed matches", no error). Now: tile = anchor's closest `<li>`, names from `__name` nodes → old whole-team-link text → the anchor's "A v B" label; the page's horizontal `match-ticker-tile` strip (today's/live matches regardless of selected day) is skipped; deduped by matchId. |
