/**
 * Surfaces shortfoot's RSS-derived news summaries and football-data.org
 * fixtures so the `/fifa-wc26` team detail sheet can render per-team
 * editorial + match data without re-ingesting any of it. Shortfoot shares
 * vizmaya-fyi's Supabase project, so the existing service client is reused.
 *
 * Team-name → entity match uses the same slug normalization as shortfoot's
...
export interface ShortfootFixture {
  id: string
  kickoffAt: string
  status: string
  homeTeam: string
...
export interface ShortfootTeamData {
  news: ShortfootNewsItem[]
  fixtures: ShortfootFixture[]
}

const EMPTY: ShortfootTeamData = { news: [], fixtures: [] }
const NEWS_LIMIT = 8
const FIXTURES_LIMIT = 10

// FIFA-name → shortfoot-slug fixups for the few names where football-data.org
// and FIFA disagree. Slugs on the right must match shortfoot's normalized
...
const TEAM_ALIASES: Record<string, string> = {
  ...
  turkiye: 'turkey',
  // FIFA "USA" / "United States" — shortfoot slug from football-data.org tends
...
async function resolveTeamEntity(
  ...
  const { data: bySlug, error: slugErr } = await sb
    ...
    .limit(1)
  if (slugErr) {
    console.warn(`[shortfoot] entity slug lookup failed for ${teamName}: ${slugErr.message}`)
    return null
  }
  ...
  const { data: byCountry, error: countryErr } = await sb
    ...
    .limit(1)
  if (countryErr) {
    console.warn(`[shortfoot] entity country lookup failed for ${teamName}: ${countryErr.message}`)
    return null
  }
...
async function fetchNewsForEntity(
  ...
  const { data, error } = await sb
    ...
    .limit(NEWS_LIMIT)
  if (error) {
    console.warn(`[shortfoot] news fetch failed for entity ${entityId}: ${error.message}`)
    return []
  }
...
async function fetchFixturesForEntity(
  ...
  if (error) {
    console.warn(`[shortfoot] fixtures fetch failed for entity ${entityId}: ${error.message}`)
    return []
  }
...
export async function getShortfootDataForTeam(
  teamName: string,
): Promise<ShortfootTeamData> {
