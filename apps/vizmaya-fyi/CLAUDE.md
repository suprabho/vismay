...
- **Frontend:** server components under [app/ai-data-centers/daily/](app/ai-data-centers/daily/) render one edition row to static HTML; the only client code is the slide-over `StoryPanel` (delegated `data-panel` handler, `#p=layer:semi` deep links), the canvas `GeoMap` (land mask shipped as a flat array) and the scroll-spy nav. No Mapbox, no ECharts, no client fetch. Tokens extend [app/ai-data-centers/theme.ts](app/ai-data-centers/theme.ts) (`energy`, `down`, composition hues, map dots) with a light theme; fonts Newsreader + IBM Plex Sans/Mono via `next/font/google`.
