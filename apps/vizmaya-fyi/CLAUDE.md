...
- **Stock importer:** [scripts/ai-data-centers/import-stock-prices.ts](scripts/ai-data-centers/import-stock-prices.ts) (`pnpm ai-data-centers:import-stocks`) — routes by the `dc_stocks.market` column: **US tickers → the massive.com market-data REST API** (Polygon-compatible `/v2/aggs/ticker/{t}/range/1/day/{from}/{to}`, `Authorization: Bearer $MASSIVE_API_TOKEN`; note massive.com returns fractional split-adjusted volume, so it's rounded to the `bigint` column); **international tickers (TW/KR/JP/NL/HK) → Twelve Data's `time_series` API** (`$TWELVEDATA_API_KEY`), since massive.com is US-only. The Twelve Data MIC is derived from `market` (TW→XTAI, KR→XKRX, JP→XTKS, NL→XAMS, HK→XHKG) paired with the numeric code, so `005930.KS` → symbol `005930` on `XKRX`. (Stooq was tried first for the international leg but bot-gates GitHub's datacenter IPs.) Both normalise to the same rows and upsert on `(ticker, trade_date)` behind a shared run-wide 429 cooldown — Twelve Data's free tier signals limits with `code:429` in a 200 body, which the fetcher treats as a rate-limit. Flags: `--full` (~5y) / `--days N` / `--ticker` / `--dry-run`. Cron: [.github/workflows/import-dc-stock-prices.yml](../../.github/workflows/import-dc-stock-prices.yml) (22:45 UTC Mon–Fri, after US close ⇒ same-day Asia/EU sessions included). **First deploy: apply migration 065, add the `MASSIVE_API_TOKEN` + `TWELVEDATA_API_KEY` secrets, then dispatch the workflow once with `full_backfill=true`.** Note: api.massive.com, api.twelvedata.com and news.google.com are all unreachable from the dev sandbox proxy — run these in Actions (same contingency as epoch.ai / iea.org). massive.com also ships an MCP server ([massive-com/mcp_massive](https://github.com/massive-com/mcp_massive)) for interactive queries, but the cron uses the REST API directly.
