...
import { aiDataCentersThemeOverrides, type AiDataCentersTheme } from '../theme'
...
export function editionMetadata(e: DcEditionWithContent, opts: { alias?: boolean; noindex?: boolean } = {}): Metadata {
  const path = opts.alias ? '/ai-data-centers/daily' : `/ai-data-centers/daily/${e.date}`
  ...
  return {
    title,
    description,
    alternates: { canonical: `/ai-data-centers/daily/${e.date}` },
