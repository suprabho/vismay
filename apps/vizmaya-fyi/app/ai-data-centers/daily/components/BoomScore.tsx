...
import type { EditionMoodCounts } from '@vismay/content-source/dcEditionTypes'
 ...
 * signed reading beneath it and the word and counts under the ring. Server
 * component: the ring's canvas (BoomRing) is the only client code, and the
 * dots painted here stand in for it until its first frame.
 */
export default function BoomScore({ score, counts }: Props) {
  ...
  return (
    ...
    >
      ...
      <figcaption className="boomscore-cap">
        ...
        {/* Each phrase stays whole, so a narrow caption breaks only at the dots. */}
        <span className="boomscore-counts">
          <span>{counts.boom} boom</span> · <span>{counts.doom} doom</span> · <span>{counts.neutral} neutral</span>
        </span>
        <a className="boomscore-go" href="#glance">
          <span>Chapter I</span> · <span>Doom v Boom →</span>
        </a>
      </figcaption>
    </figure>
  )
