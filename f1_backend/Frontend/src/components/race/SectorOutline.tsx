...
import { buildProjector } from '../../utils/trackProjection';
...
function pathFromSlice(
  ox: number[], oy: number[], from: number, to: number,
 ...
 * boundaries are missing) or as three coloured segments.
 *
 * The halo (faint thick stroke) is always drawn underneath for depth.
 */
export function SectorOutline({ circuit, viewportW, viewportH, sectorColors }: Props) {
  ...
  if (!sb || sb.index1 <= 0 || sb.index2 <= sb.index1 || sb.index2 >= ox.length - 1) {
    return (
      <>
        <path d={projector.outlinePath} fill="none" stroke="#9CA3AF" strokeWidth={6}
              strokeLinejoin="round" strokeLinecap="round" opacity={0.35} />
        <path d={projector.outlinePath} fill="none" stroke={COLOR_MAP.neutral} strokeWidth={1.5}
  ...
  // Close back to point 0 with a final L command
  ...
  return (
    <>
      ...
      <path d={s1} fill="none" stroke={stroke(sectorColors[0])} strokeWidth={2.5}
            strokeLinejoin="round" strokeLinecap="round" />
      <path d={s2} fill="none" stroke={stroke(sectorColors[1])} strokeWidth={2.5}
            strokeLinejoin="round" strokeLinecap="round" />
      <path d={s3} fill="none" stroke={stroke(sectorColors[2])} strokeWidth={2.5}
            strokeLinejoin="round" strokeLinecap="round" />
      ...
      {(() => {
        ...
        return (
          <>
            <circle cx={p1.sx} cy={p1.sy} r={3.5} fill="#fff" stroke="#111" strokeWidth={1} />
            <circle cx={p2.sx} cy={p2.sy} r={3.5} fill="#fff" stroke="#111" strokeWidth={1} />
            <text x={p1.sx + 6} y={p1.sy - 6} fontSize={9} fontFamily="monospace"
                  fill="#374151" fontWeight={700}>S1/S2</text>
            <text x={p2.sx + 6} y={p2.sy - 6} fontSize={9} fontFamily="monospace"
                  fill="#374151" fontWeight={700}>S2/S3</text>
          </>
        );
      })()}
    </>
