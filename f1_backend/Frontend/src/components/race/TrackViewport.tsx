...
import type {
  CarPositionTrack, CircuitGeometry, ProcessedLap, SectorBests,
} from '../../config/api';
...
interface Props {
  ...
  /** Called by parent when it expects redraws (typically every animation frame). */
  redrawSignal:   number;
}
...
export function TrackViewport({
  ...
  sectorBests, currentLap, aggregates, currentTimeRef, redrawSignal,
}: Props) {
  ...
  useEffect(() => {
    ...
    for (const d of drivers) {
      ...
      handles.circle.setAttribute('stroke', frame.status === 1 ? '#F59E0B' : (focused ? '#111' : '#ffffff'));
  ...
  return (
    <div className="relative border border-neutral-200 bg-neutral-50 flex-1 w-full h-full min-h-0">
      ...
      >
        ...
        {/* Corner labels */}
        {projector && circuit.corners.map(corner => {
          ...
          return (
            <g key={d.driverNumber}>
              ...
              <text
                ...
                x={-100} y={-100}
                fontSize={9} fontWeight={700} fill={colour}
      ...
      <div className="absolute top-3 left-3 flex flex-col gap-1 pointer-events-none">
        <span className="font-mono text-[9px] uppercase tracking-widest text-neutral-400 bg-white/70 px-1.5 py-0.5">
          {circuit.circuitName || circuit.circuitKey}
        </span>
        <span className="font-mono text-[9px] text-neutral-400 bg-white/70 px-1.5 py-0.5">
          {visibleDrivers.size} car{visibleDrivers.size === 1 ? '' : 's'} on track
          {focusedDriver != null && ` · focus #${focusedDriver}`}
        </span>
      </div>

      <div className="absolute top-3 right-3 flex items-center gap-3 bg-white/80 px-2 py-1 pointer-events-none">
        <span className="flex items-center gap-1 font-mono text-[9px] text-neutral-500">
          <span className="inline-block w-2 h-2 bg-neutral-400" /> on track
        </span>
        <span className="flex items-center gap-1 font-mono text-[9px] text-neutral-500">
          <span className="inline-block w-2 h-2 bg-neutral-400 opacity-40" /> pit
        </span>
        <span className="flex items-center gap-1 font-mono text-[9px] text-neutral-500">
          <span className="inline-block w-2 h-2 border-2 border-amber-500" /> off
        </span>
      </div>

      {/* Sector legend (when boundaries available) */}
      {circuit.sectorBoundaries && (
        <div className="absolute bottom-3 right-3 flex items-center gap-3 bg-white/80 px-2 py-1 pointer-events-none">
          <span className="flex items-center gap-1 font-mono text-[9px] text-neutral-500">
            <span className="inline-block w-3 h-0.5" style={{ background: '#22C55E' }} /> driver PB
          </span>
          <span className="flex items-center gap-1 font-mono text-[9px] text-neutral-500">
            <span className="inline-block w-3 h-0.5" style={{ background: '#A855F7' }} /> session purple
          </span>
        </div>
      )}
    </div>
