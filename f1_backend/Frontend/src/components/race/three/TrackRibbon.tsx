...
import { SECTOR_COLOR_HEX, type SectorColor } from '../../../utils/sectorClassification';
...
/**
 * A constant-width track ribbon built from the circuit outline centreline. Sector
 * colouring is baked as vertex colours (one draw call) and updated in place when
 * the focused driver's sector classification changes — geometry is built once.
 */
export function TrackRibbon({ circuit, projector, sectorColors }: Props) {
  // Geometry + per-vertex sector slot (0/1/2), rebuilt only when the track changes.
  const built = useMemo(() => {
    const pts = projector.outlineWorld;
    const n = pts.length;
    if (n < 3) return null;

    // Width scaled to track size so the ribbon reads as a road at any zoom.
    ...
    for (let i = 0; i < n; i++) {
      ...
      tangent.y = 0; // keep the ribbon width horizontal
  ...
  // Recolour vertices in place when the sector classification changes.
  useEffect(() => {
    if (!built) return;
  ...
  // Dispose GPU geometry when it is replaced or the component unmounts.
  useEffect(() => () => built?.geometry.dispose(), [built]);

  if (!built) return null;
  // Unlit so the ribbon shows its exact colour regardless of lighting/normals.
  return (
