/**
 * GET / PUT the per-story map.yaml that overrides camera + pins + layers
 * per (parentIndex, subIndex?) without touching the base config.yaml.
 *
 * GET → 200 { raw: string | null }
 * PUT → 200 { ok: true }   body: { raw: string | null }
 *
 * Admin-auth gated. The body is opaque YAML — parsing happens in
 * `lib/storyMapOverrides.ts` at config-load time, so a malformed save
 * silently falls through to the base config rather than crashing.
 */
