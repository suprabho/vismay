...
import { EPSTEIN_THEME_DEFAULTS, type EpsteinTheme } from '@/app/epstein/theme'

const SAFE_SLUG = /^[a-zA-Z0-9_-]+$/
const HEX = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/
const ALLOWED_KEYS = Object.keys(EPSTEIN_THEME_DEFAULTS) as (keyof EpsteinTheme)[]
...
export async function PUT(
  ...
  for (const [key, value] of Object.entries(body.theme as Record<string, unknown>)) {
    if (!ALLOWED_KEYS.includes(key as keyof EpsteinTheme)) continue
