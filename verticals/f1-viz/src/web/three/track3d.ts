/**
 * World-space projector for the 3D track view.
 *
 * Ported verbatim from the f1_backend donor (`src/utils/track3d.ts`) — pure math.
 * Only the CircuitGeometry import path changed (donor `../config/api` →
 * the vertical's replay types).
 */
 ...
 * projector's rotation/recentre so both views agree on orientation, but maps
 * onto the XZ ground plane with Y as (exaggerated) elevation.
 */
