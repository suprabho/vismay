...
import { createFixtureDataSource, type ReplayDataSource } from './dataSource'
import { useReplayData } from './useReplayData'
import { computeLiveStandings, findFrameIndex, timeAtLapStart } from './trackProjection'
import type { ProcessedLap } from './types'

interface RaceReplayProps {
  /** Session reference passed to the data source (resolves to a fixture / table row). */
  sessionRef: string
  /**
   * Data source to load from. Defaults to a fixture-backed source that falls
   * back to the shared `demo` fixture. The `f1:race-replay` module injects an
   * inline source when the layer carries its telemetry in the config.
   */
  dataSource?: ReplayDataSource
  /** Start playing as soon as data loads (e.g. catalog autoplay / scroll mount). */
  autoPlay?: boolean
}

export function RaceReplay({ sessionRef, dataSource, autoPlay = false }: RaceReplayProps) {
  ...
  const fallbackSource = useMemo(() => createFixtureDataSource({ fallbackRef: 'demo' }), [])
  const source = dataSource ?? fallbackSource
  ...
  }, [race.bounds, race.tracks, race.totalLaps, autoPlay])
